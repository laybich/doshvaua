...
<html lang="ru">
	...
	<head>
		...
		<link rel="icon" href="/favicon.ico">
		<link rel="stylesheet" href="product.min.css">
	</head>
	...
	<body>
		<main>
			...
			<section id="catalog">
				<ul class="bold d-md-flex justify-content-around tabs">
					<li><a href="/catalog/#catalog">Мужские кроссовки</a></li>
					<li><a href="/catalog/?tabID=2#catalog">Женские кроссовки</a></li>
					<li><a href="/catalog/?tabID=3#catalog">Кроссовки для мальчиков</a></li>
				</ul>

				<div class="container">
					<p class="subtitle">О П И С А Н И Е</p>				
					<h1 id="title"></h1>
					...
					<div class="card">
						<div class="row">
							<div class="col-lg-6">
								...
								<ul class="bold d-flex justify-content-around tabs tab-s">
									<li data-toggle="tab" data-target="0" class="active"><a href="#">Описание</a></li>
									<li data-toggle="tab" data-target="1"><a href="#">Характеристики</a></li>
								</ul>
								<div id="0" class="card-s active"></div>
								<div id="1" class="card-s params justify-content-between">
									<ul class="name"></ul>
									<ul class="value text-right"></ul>
								</div>
							</div>
							<div class="col-lg-6">
								<div class="form-order">
									<h3>Выберите размер:</h3>
									<input type="number" name="size" placeholder="Доступные размеры в характеристиках">
		...
		<script src="/js/tabs.min.js" defer="defer"></script>
		<script src="/js/basket.min.js" defer="defer"></script>
		<script>
			$(function()
			{
				// Fill offer
				var shoe = getShoe('<?php echo $_GET['offerID'] ?>')
				var pictures = shoe.querySelectorAll('picture')

				for(let i = 0; i < pictures.length; i++)
				{
					$('.owl-carousel').append(`<img class="owl-lazy" data-src="${ pictures[i].innerHTML }">`)
				}

				document.querySelector('h1').innerHTML = `${ shoe.querySelector('name').innerHTML }`
				document.querySelector('title').innerHTML = shoe.querySelector('name').innerHTML
				document.querySelector('[id="0"]').innerHTML = shoe.querySelector('description').textContent

				let params = shoe.querySelectorAll('param')

				for(let i = 0; i < params.length; i++)
				{
					$('.params .name').append(`<li>${ params[i].getAttribute('name') }</li>`)
					$('.params .value').append(`<li>${ params[i].innerHTML }</li>`)
				}

				$('main .btn-card').append(` (${ shoe.querySelector('price').innerHTML } грн)`)
				...
				// Add to basket
				$('.card .btn-card').click(function(e) {
					e.preventDefault()
					if($('input[name="size"]')[0].value)
					{
						$('.body__basket ul').append(
							`<li id="${ id }">
								<img src="${ pictures[0].innerHTML }">
						...
						let product = {
							id: <?php echo $_GET['offerID'] ?>,
							size: $('input[name="size"]')[0].value,
							price: shoe.querySelector('price').innerHTML
						}

						total_price += Number(product.price)
						IDs += (`${ product.id } : ${ product.size }, `)
